...
func listToPoint(expr expreduceapi.Ex, gtx *layout.Context) (float32, float32, error) {
...
func setDirective(dir expreduceapi.Ex, style *Style, gtx *layout.Context) error {
...
func renderPrimitive(primitive expreduceapi.Ex, style *Style, gtx *layout.Context) error {
...
func drawAxis(ex *atoms.Expression, gtx *layout.Context) {
}

func Graphics(expr *atoms.Expression, st *Style, gtx *layout.Context) layout.Widget {
	graphics, ok := atoms.HeadAssertion(expr, "System`Graphics")
	if !ok {
		fmt.Printf("trying to render a non-Graphics[] expression: %v\n", expr)
		return nil
	}
	...
	renderPrimitive(graphics.GetPart(1), &style, gtx)
